...
export const createEditRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const challan = await prisma.challan.findUnique({
      where: { id: parseInt(challanId as string) }
    });
    ...
    const existingRequest = await prisma.challanEditRequest.findFirst({
      where: { challanId: parseInt(challanId as string), status: 'PENDING' }
    });
    ...
    const editRequest = await prisma.challanEditRequest.create({
      data: {
        challanId: parseInt(challanId as string),
...
export const getEditRequests = async (req: Request, res: Response): Promise<void> => {
  try {
    const requests = await prisma.challanEditRequest.findMany({
      include: {
        challan: {
...
export const resolveEditRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const request = await prisma.challanEditRequest.findUnique({
    ...
    if (action === 'REJECT') {
      const updatedRequest = await prisma.challanEditRequest.update({
        where: { id: request.id },
        data: {
          status: 'REJECTED',
          resolvedById: userId,
